...
const count = (target, input) => {
  // Solution code here...
  let myArr = input.reduce((acc, idx) => acc.concat(idx), []);
  let count = 0;
  myArr.map(value => {
    if (value == target) {
      count += 1;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, arr) => acc += arr.reduce((acc2, value) => acc2 += value), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let myArr = [];
  input.forEach(arr => {
    myArr.push(arr.filter(val => typeof (val) === 'number' && val % 5 === 0))
  })
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let myArr = data.filter(character => character.gender === 'male' || character.gender === 'female')
  let names = myArr.map(character => {
...
let findShortest = (data) => {
  // Solution code here...
  let myArr = data.reduce((acc, val) => {
    if (acc.name.length >= val.name.length) {
      acc = val
    }
    return acc;
  }, data[0])
